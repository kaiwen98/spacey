...
LOCATION, CHECKWHAT, SUBSCRIBE_MAIN, SUBSCRIBE_TIME, SUBSCRIBE_TYPE, SUBSCRIBE_LOCATION, UNSUBSCRIBE_TYPE, SETSPACEYVALUE= range(8)
red_alert = "\ud83d\udd34"
yellow_alert = "\ud83d\udfe1"
green_alert = "\ud83d\udfe2"
seats_emoji = "\ud83e\ude91"
...
def start(update, context):
    ...
    if user_id not in users_info.keys():
        dict_object = {"name": name, "username": username, "daily_notifications": "0", "full_notifications": "0", "flag":"0","daily_time":"0"}
...
def check_what(update, context):
    ...
    if option == 'Occupancy':
        ...
        data = cfg.database.client.hgetall("NUS_"+location+"_totalvisitors")
...
def hourly_update(context):
    ...
    for location in locations_list:
        ...
        cfg.database.client.hmset('NUS_'+location+'_totalvisitors',new_entry)    
...
def main():
    ...
    spaceybot = MQBot('<TELEGRAM API KEY>', request=request, mqueue=q)

...
//spacey
std::string BLEAdvertisedDevice::getClusterNum(){
	return m_clusterNum;
}
	...
	while(!finished) {
		...
		if (length != 0) { // A length of 0 indicates that we have reached the end.
			...
			switch(ad_type) {
				...
				case ESP_BLE_AD_TYPE_NAME_CMPL:  {   // Adv Data Type: 0x09, 0x08
				...
				case ESP_BLE_AD_TYPE_NAME_SHORT: {
					setClusterNum(std::string(reinterpret_cast<char*>(payload), length));
					break;
				}
...
//spacey
void BLEAdvertisedDevice::setClusterNum(std::string clusternum){
	m_clusterNum = clusternum;
	/*
	std::stringstream x(clusternum);
	x >> m_clusterNum;
	log_d("Set cluster name to %d", m_clusterNum);
	*/
}

...
void BLEAdvertisementData::setName(std::string name) {
	...
	log_d("name: ", name.c_str());
} // setName


//Spacey
void BLEAdvertisementData::setClusterNum(std::string clusterNum) {
	log_d("BLEAdvertisementData", ">> setClusterNum: %d", clusterNum);
	log_d("Cluster Num: %d", clusterNum);
	char cdata[2];
	cdata[0] = clusterNum.length() + 1;
	cdata[1] = ESP_BLE_AD_TYPE_NAME_SHORT;  // 0x08
	addData(std::string(cdata, 2) + clusterNum);
	log_d("BLEAdvertisementData", "<< setClusterNum: %s", clusterNum.c_str());
	log_d("Cluster Num: %s", clusterNum);
} // setName

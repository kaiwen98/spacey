...
// ignore it.
					...
					int pass =  (m_clusterNum.compare( advertisedDevice->m_clusterNum )?false:true) && !found;
					/*
					if (!found && !m_clusterNum.compare( advertisedDevice->m_clusterNum )) {   // If we have previously seen this device, don't record it again.
					...
					else if (m_clusterNum.compare( advertisedDevice->m_clusterNum )) {
						log_d("Wrong cluster ID");
					...
					*/
					...
					if (pass){
						m_scanResults.m_vectorAdvertisedDevices.insert(std::pair<std::string, BLEAdvertisedDevice*>(advertisedAddress.toString(), advertisedDevice));
						log_d("Added %s to the map: ", advertisedAddress.toString().c_str());
						//session_address_buffer.append(advertisedAddress.toString());
						if (m_pAdvertisedDeviceCallbacks) m_pAdvertisedDeviceCallbacks->onResult(*advertisedDevice);
					}
					else delete advertisedDevice;
